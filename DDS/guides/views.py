...
def edit_type(request,type_id):
    status = get_object_or_404(Type, id=type_id)  
    if request.method == 'POST':
        form = TypeForm(request.POST, instance=status)  
        if form.is_valid():
            form.save() 
            return redirect('guides:types_list') 
    else:
        form = TypeForm(instance=status)  

    return render(request, 'guides/edit_guide.html', {'form': form})
...
def delete_type(request, type_id):
    ...
    if request.method == 'POST':
        type.delete()  
        return redirect('guides:types_list')  
    return render(request, 'guides/types/delete_type.html', {'type': type})
...
def delete_status(request, status_id):
    ...
    if request.method == 'POST':
        status.delete()  
        return redirect('guides:statuses_list')  
    return render(request, 'guides/statuses/delete_status.html', {'status': status})
